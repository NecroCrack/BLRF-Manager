...
// Tente de charger la configuration du site Figma si elle existe, sinon utilise un objet vide (fallback).
...
try {
  ...
  console.warn("Le fichier Figma site.json est introuvable, utilisation d'une configuration vide.");
}

// Vite config — https://vitejs.dev/config/
export default defineConfig(({ mode }) => {
// ... conservez le reste de votre fichier exactement tel quel à partir d'ici ...
  ...
  return {
    ...
    server: {
      host: '0.0.0.0',
      port: parseInt(process.env.FRONTEND_PORT || '8443'),
      strictPort: true,
      watch: { ignored: ['**/.figma/**'] },
      proxy: {
        '/api': {
